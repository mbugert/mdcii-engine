...
func (c *Cod) constantAssignment(line string) (bool, error) {
	line = strings.ReplaceAll(line, " ", "")
	matches := regexSearch(`(\w+)\s*=\s*((?:\.+\d+|\+|\w+)+)`, line)
	if len(matches) > 0 {
		isMath := strings.Contains(matches[1], "+")
		constant := constantType{
			key:   matches[0],
			value: matches[1],
		}
		// HAUSWACHS = Nummer
		if constant.key == NUMBER_INCREMENT {
		...
		fmt.Println(c.Intern.constants.Variable)
		return true, nil
	}
